  ...
  const syncWithGoogleSheets = useCallback(async () => {
    ...
    try {
      ...
      if (Array.isArray(txData)) {
        // Sanitize data from GAS (ensure amounts are numbers and dates are valid)
        ...
        <div className="lg:hidden fixed inset-0 z-40 bg-[#1D1B20]/40 backdrop-blur-sm" onClick={() => setIsMobileMenuOpen(false)}>
          <div className="absolute top-[80px] left-3 right-3 bg-white rounded-[32px] p-4 flex flex-col gap-2">
            {['dashboard', 'reports', 'settings'].map(t => (
              <button key={t} onClick={() => { setActiveTab(t); setIsMobileMenuOpen(false); }} className={`flex items-center gap-4 font-black p-4 rounded-[24px] ${activeTab === t ? 'bg-[#DDFD54]' : 'text-[#7A7585]'}`}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 md:gap-8 animate-in fade-in duration-500">
             <div className="lg:col-span-8 space-y-6 md:space-y-8">
                <div className="pt-2 md:pt-4">
                  <h2 className="text-3xl md:text-6xl font-black tracking-tighter text-[#1D1B20] leading-none">Smart Ledger<br/><span className="text-[#AE88F9]">For WTR.</span></h2>
                  <p className="text-[#7A7585] font-black mt-3 md:mt-6 uppercase tracking-widest text-[9px] md:text-xs">Manage your cloud database with photos</p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
                   <div className="md:col-span-2 bg-[#AE88F9] p-6 md:p-10 rounded-[32px] md:rounded-[56px] text-white shadow-2xl relative overflow-hidden group">
                      <p className="text-white/60 font-black text-[9px] md:text-xs uppercase tracking-widest mb-1 md:mb-2">Net Cash Balance</p>
                   ...
                   <div className="bg-[#DDFD54] p-6 md:p-10 rounded-[32px] md:rounded-[56px] flex flex-col justify-between shadow-sm">
                      ...
                      <div><p className="text-black/60 font-black text-[9px] md:text-xs uppercase tracking-widest mb-1">Income</p><h4 className="text-xl md:text-2xl font-black truncate">{formatCurrency(totals.income)}</h4></div>
                   </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4 md:gap-6">
                   <button onClick={() => handleOpenModal('income')} className="bg-[#1D1B20] text-white p-6 md:p-12 rounded-[32px] md:rounded-[56px] flex flex-col justify-between min-h-[140px] md:min-h-[220px] shadow-2xl hover:scale-[1.02] transition-transform">
                      ...
                      <div className="text-left font-black text-lg md:text-3xl tracking-tighter uppercase">+ Income</div>
                   </button>
                   <button onClick={() => handleOpenModal('expense')} className="bg-white border-2 border-[#EAE3F4] p-6 md:p-12 rounded-[32px] md:rounded-[56px] flex flex-col justify-between min-h-[140px] md:min-h-[220px] shadow-md hover:scale-[1.02] transition-transform">
                      ...
                      <div className="text-left font-black text-lg md:text-3xl tracking-tighter uppercase">- Expense</div>
                   </button>
                </div>
             </div>
             ...
             <div className="lg:col-span-4 bg-white rounded-[32px] md:rounded-[56px] shadow-2xl flex flex-col h-[600px] lg:h-[750px] overflow-hidden border border-[#EAE3F4] animate-in slide-in-from-right-8 duration-700">
                <div className="p-6 md:p-10 border-b border-[#F8F7FA] flex justify-between items-center">
                   <h3 className="font-black text-lg md:text-2xl tracking-tighter uppercase">Activity</h3>
                   <button onClick={() => setIsHistoryOpen(true)} className="text-[#AE88F9] font-black text-xs md:text-sm">VIEW ALL</button>
                </div>
        ...
        {activeTab === 'reports' && (
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
             <div className="flex justify-between items-end pt-4">
               <div>
                 <h2 className="text-3xl md:text-5xl font-black text-[#1D1B20] tracking-tighter">Finance Review</h2>
                 <p className="text-[#7A7585] text-xs md:text-sm font-bold mt-2">สรุปรายการรายสัปดาห์และรายเดือน</p>
               </div>
               <div className="bg-white p-1 rounded-full shadow-sm border border-[#EAE3F4] flex">
                 {['daily', 'weekly', 'monthly'].map(p => (
                   <button key={p} onClick={() => setReportPeriod(p)} className={`px-3 md:px-5 py-1.5 md:py-2 font-black text-[10px] md:text-xs rounded-full transition-all ${reportPeriod === p ? 'bg-[#1D1B20] text-[#DDFD54]' : 'text-[#7A7585]'}`}>
                     {p.toUpperCase()}
                   </button>
                 ))}
               </div>
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="bg-white border-2 border-[#EAE3F4] p-6 md:p-10 rounded-[32px] md:rounded-[48px] h-[350px] md:h-[450px] flex flex-col shadow-sm">
                   <h3 className="font-black text-lg md:text-2xl mb-6 md:mb-8 flex items-center gap-2"><ArrowUpRight className="text-emerald-500" /> TOP INCOME</h3>
                   <div className="flex-1 space-y-4 md:space-y-6 overflow-y-auto custom-scrollbar pr-2">
                      ...
                      {breakdownData.income.items.length === 0 && <p className="text-center py-20 text-[#7A7585] font-black opacity-30">NO INCOME DATA</p>}
                   </div>
                </div>
                <div className="bg-white border-2 border-[#EAE3F4] p-6 md:p-10 rounded-[32px] md:rounded-[48px] h-[350px] md:h-[450px] flex flex-col shadow-sm">
                   <h3 className="font-black text-lg md:text-2xl mb-6 md:mb-8 flex items-center gap-2"><ArrowDownRight className="text-[#AE88F9]" /> TOP EXPENSE</h3>
                   <div className="flex-1 space-y-4 md:space-y-6 overflow-y-auto custom-scrollbar pr-2">
                      ...
                      {breakdownData.expense.items.length === 0 && <p className="text-center py-20 text-[#7A7585] font-black opacity-30">NO EXPENSE DATA</p>}
                   </div>
                </div>
             </div>
          </div>
        ...
        {activeTab === 'settings' && (
          <div className="max-w-4xl mx-auto space-y-6 md:space-y-8 animate-in fade-in duration-500">
             <div className="text-center pt-4 md:pt-8 px-4">
               <h2 className="text-2xl md:text-4xl font-black tracking-tighter">System Control</h2>
               <p className="text-[#7A7585] font-bold mt-2 text-xs md:text-sm">จัดการหมวดหมู่และการเชื่อมต่อ Cloud</p>
             </div>
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                <div className="bg-[#1D1B20] p-8 md:p-10 rounded-[32px] md:rounded-[48px] text-white shadow-2xl relative overflow-hidden">
                   <h3 className="font-black text-lg md:text-xl mb-6 relative z-10">CLOUD CONNECT</h3>
                   <div className="space-y-4 relative z-10">
                      <div className="bg-white/10 p-4 rounded-3xl border border-white/10">
                        <p className="text-[9px] font-black text-[#DDFD54] mb-1">GOOGLE SHEETS API URL</p>
                        <p className="text-[10px] truncate opacity-60 font-mono italic">{API_URL}</p>
                      </div>
                      <button onClick={syncWithGoogleSheets} className="w-full bg-[#DDFD54] text-[#1D1B20] py-3 md:py-4 rounded-2xl md:rounded-3xl font-black text-xs md:text-sm hover:scale-[1.02] transition-transform">SYNC DATA NOW</button>
                      <button onClick={handleMigration} className="w-full bg-white/10 text-white py-3 md:py-4 rounded-2xl md:rounded-3xl font-black text-xs md:text-sm border border-white/20 hover:bg-white/20">UPLOAD LOCAL TO CLOUD</button>
                   </div>
                ...
                <div className="bg-white border-2 border-[#EAE3F4] p-8 md:p-10 rounded-[32px] md:rounded-[48px] shadow-sm">
                   <h3 className="font-black text-lg md:text-xl mb-6">DATA RESET</h3>
                   <div className="space-y-4">
                      <p className="text-[10px] text-[#7A7585] font-medium leading-relaxed">การล้างข้อมูลจะลบรายการในเมมโมรี่ของเบราว์เซอร์เท่านั้น ข้อมูลบน Google Sheets ของคุณจะยังคงปลอดภัย</p>
                      <button onClick={() => {if(window.confirm('Clear Cache?')) { localStorage.clear(); window.location.reload(); }}} className="w-full bg-rose-50 text-rose-500 py-3 md:py-4 rounded-2xl md:rounded-3xl font-black text-xs md:text-sm border border-rose-100 hover:bg-rose-100">CLEAR LOCAL CACHE</button>
                   </div>
                </div>
             </div>
             ...
             <div className="bg-white border-2 border-[#EAE3F4] p-8 md:p-10 rounded-[32px] md:rounded-[48px] shadow-sm">
                <div className="mb-8">
                  <h3 className="font-black text-xl md:text-2xl tracking-tighter uppercase">Category Registry</h3>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-10">
                   <div>
                      <h4 className="font-black text-emerald-500 text-[10px] md:text-xs mb-4 tracking-widest uppercase flex items-center gap-2"><ArrowUpRight size={18} /> Income Sources</h4>
                      <div className="flex flex-wrap gap-2">
                        ...
                        <button onClick={() => {setNewCategoryType('income'); setIsAddCategoryModalOpen(true);}} className="bg-[#DDFD54] px-3 md:px-4 py-1.5 md:py-2 rounded-xl md:rounded-2xl text-[9px] md:text-xs font-black">+ ADD</button>
                      </div>
                   </div>
                   <div>
                      <h4 className="font-black text-[#AE88F9] text-[10px] md:text-xs mb-4 tracking-widest uppercase flex items-center gap-2"><ArrowDownRight size={18} /> Expense Types</h4>
                      <div className="flex flex-wrap gap-2">
                        ...
                        <button onClick={() => {setNewCategoryType('expense'); setIsAddCategoryModalOpen(true);}} className="bg-[#AE88F9] text-white px-3 md:px-4 py-1.5 md:py-2 rounded-xl md:rounded-2xl text-[9px] md:text-xs font-black">+ ADD</button>
                      </div>
                   </div>
                </div>
             </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-end md:items-center justify-center p-2 md:p-4 bg-[#1D1B20]/60 backdrop-blur-sm animate-in fade-in duration-300">
           <div className="bg-white w-full max-w-xl rounded-t-[32px] md:rounded-[64px] shadow-2xl overflow-hidden flex flex-col animate-in slide-in-from-bottom-8 duration-500 max-h-[90vh]">
              <div className={`p-6 md:p-14 flex justify-between items-center ${modalType === 'income' ? 'bg-[#DDFD54]' : 'bg-[#AE88F9] text-white'}`}>
                 <h2 className="text-2xl md:text-4xl font-black uppercase tracking-tighter leading-none">New Entry<br/><span className="text-sm md:text-2xl opacity-60">Record {modalType}</span></h2>
              ...
              <form onSubmit={handleFormSubmit} className="p-6 md:p-12 space-y-4 md:space-y-6 overflow-y-auto custom-scrollbar">
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-1.5 md:space-y-2"><label className="text-[9px] md:text-[10px] font-black uppercase opacity-40 ml-1">Date</label><input type="date" value={formData.date} onChange={e => setFormData({...formData, date: e.target.value})} className="w-full bg-[#F8F7FA] p-4 md:p-5 rounded-2xl md:rounded-3xl outline-none font-black text-sm" /></div>
                    <div className="space-y-1.5 md:space-y-2"><label className="text-[9px] md:text-[10px] font-black uppercase opacity-40 ml-1">Type</label><select value={formData.category} onChange={e => setFormData({...formData, category: e.target.value})} className="w-full bg-[#F8F7FA] p-4 md:p-5 rounded-2xl md:rounded-3xl outline-none font-black text-sm appearance-none">{categories[modalType].map(c => <option key={c} value={c}>{c}</option>)}</select></div>
                 </div>
                 <input type="text" placeholder="PARTNER NAME / CUSTOMER" value={formData.partyName} onChange={e => setFormData({...formData, partyName: e.target.value})} className="w-full bg-[#F8F7FA] p-5 md:p-6 rounded-2xl md:rounded-[24px] outline-none font-black text-xs md:text-sm focus:border-black border-2 border-transparent transition" />
                 <input type="text" placeholder="ITEM DESCRIPTION" value={formData.itemName} onChange={e => setFormData({...formData, itemName: e.target.value})} className="w-full bg-[#F8F7FA] p-5 md:p-6 rounded-2xl md:rounded-[24px] outline-none font-black text-xs md:text-sm focus:border-black border-2 border-transparent transition" />
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <input type="number" placeholder="PRICE/UNIT" value={formData.unitPrice} onChange={e => setFormData({...formData, unitPrice: e.target.value})} className="w-full bg-[#F8F7FA] p-5 md:p-6 rounded-2xl md:rounded-3xl outline-none font-black text-xs md:text-sm" />
                    <input type="number" placeholder="QTY" value={formData.quantity} onChange={e => setFormData({...formData, quantity: e.target.value})} className="w-full bg-[#F8F7FA] p-5 md:p-6 rounded-2xl md:rounded-3xl outline-none font-black text-xs md:text-sm" />
                 </div>
                 
                 <div className="space-y-3">
                    <label className="text-[9px] md:text-[10px] font-black uppercase opacity-40 ml-1">Attachments (Photos)</label>
                    <div className="flex flex-wrap gap-2">
                       ...
                       <button type="button" onClick={() => fileInputRef.current?.click()} className="w-16 h-16 md:w-20 md:h-20 rounded-xl md:rounded-2xl border-2 border-dashed border-[#EAE3F4] flex flex-col items-center justify-center text-[#7A7585] hover:bg-[#F8F7FA] transition active:scale-95">
                          <Camera size={20} />
                          <span className="text-[8px] md:text-[9px] font-black mt-1">ADD</span>
                       </button>
                    </div>
                 </div>

                 <div className="p-6 md:p-8 bg-[#1D1B20] text-center rounded-[24px] md:rounded-[32px] shadow-2xl">
                    <p className="text-[9px] md:text-[10px] font-black text-white/50 mb-1 uppercase tracking-widest">Total Amount Due</p>
      ...
      {isAddCategoryModalOpen && (
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-[#1D1B20]/40 backdrop-blur-md">
           <div className="bg-white p-8 md:p-10 rounded-[32px] md:rounded-[48px] w-full max-w-sm shadow-2xl animate-in zoom-in-95 duration-200">
              <h3 className="text-xl md:text-2xl font-black mb-6 uppercase tracking-tighter">Add {newCategoryType}</h3>
              <input type="text" autoFocus value={newCategoryName} onChange={e => setNewCategoryName(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleAddCategory()} placeholder="NAME..." className="w-full bg-[#F8F7FA] p-5 rounded-2xl md:rounded-3xl outline-none font-black text-sm mb-6" />
      ...
      {isHistoryOpen && (
        <div className="fixed inset-0 z-[150] bg-white animate-in slide-in-from-bottom duration-500 flex flex-col">
           <div className="p-6 md:p-10 border-b border-[#F8F7FA] flex justify-between items-center bg-white/50 backdrop-blur-md sticky top-0 z-20">
              <h2 className="text-2xl md:text-5xl font-black tracking-tighter uppercase">Full Statement</h2>
